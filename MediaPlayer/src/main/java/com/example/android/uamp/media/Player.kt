...
object Player : IPlayer {
    ...
    override fun start(mediaUri: Uri) = playerImpl.start(mediaUri)
...
private class PlayerImpl(private val appContext: Context) : IPlayer {
        ...
        set(value) {
            ...
            controls {
                ...
                if (playNowIdNew != null) {
                    ...
                    when (state) {
                        State.PLAY -> {
                            playFromUri(playNowIdNew, extra)
                        }
                        State.PAUSE -> {
                            playFromUri(playNowIdNew, extra)
                        }
    ...
    override fun start(mediaUri: Uri) {
        controls {
            playFromUri(mediaUri, null)
        }
    }
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        ...
        override fun onMetadataChanged(metadata: MediaMetadataCompat?) {
            val item = _liveDataPlayList.value?.find { it.id == metadata?.description?.mediaUri }
